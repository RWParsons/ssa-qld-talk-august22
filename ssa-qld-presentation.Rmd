...
Existing approaches sample coefficients from MVN for coefficients and then use those to sample the outcome.
...
```r
x <- c(rnorm(n_neg, mean = 0, sd = 1), rnorm(n_pos, mean = cohens_d, sd = 1))
...
```{r, echo=F}
sample_data <- 
  get_sample(auc=0.7, n_samples=10000, prevalence=0.05) %>%
  rename(x=predicted, y=actual)
```
...
>- How would my model (or treat-all/none strategy) perform regarding NMB?
...
screen_obj_intervention <- screen_simulation_inputs(
  n_sims = 5000, n_valid = 10000, sim_auc = seq(0.7, 0.95, 0.05), event_rate = 0.05,
  fx_nmb_training = fx_nmb_old,  # training doesn't matter since we are not using a cost_effective threshold
  fx_nmb_evaluation = list("fx_nmb_old"=fx_nmb_old, "fx_nmb_new"=fx_nmb_new),
  cutpoint_methods = c("all", "none", "youden"), meet_min_events = TRUE, cl = cl
)
...
>- What is the relative merit of improving my model's AUC by X (in \$)?
>- Has or will there research efforts towards developing models with ineffective interventions or where models are required to be unbelievably good to be effective?
